...
from ..utils.misc import set_random_seed, get_parser
...
class MRCNERTrainer:
    ...
    def train(self, model, optimizer, scheduler, train_dataloader, eval_dataloader, writer, device, model_dir):
        ...
        for epoch in range(self.num_epochs):
            ...
            eval_result = self.evaluate(model, eval_dataloader, device)
            ...
            model_name = f"model_for_conll03_p_{p:.4f}_r_{r:.4f}_f1_{f1:.4f}.pth"
            model_path = os.path.join(model_dir, model_name)
            torch.save(model.state_dict(), model_path)
            

    def evaluate(self, model, eval_dataloader, device):
        model.eval()
        eval_loss = 0
        eval_start_loss = 0
        eval_end_loss = 0
...
def train_main():
    ...
    bert_config_dir = "G:/huggingface/models/bert-large-cased"
    ...
    is_chinese = False
    num_workers = 8
    train_dataloader = get_dataloader(prefix="train", batch_size=batch_size,
                                      data_dir=data_dir, bert_dir=bert_config_dir,
                                      max_length=max_length, is_chinese=is_chinese,
                                      num_workers=num_workers)
    
    eval_dataloader = get_dataloader(prefix="dev", batch_size=batch_size,
                                      data_dir=data_dir, bert_dir=bert_config_dir,
                                      max_length=max_length, is_chinese=is_chinese,
                                      num_workers=num_workers)

    tensorboard_path = "../../data/tensorboard/conll03/"
    model_dir = "../../data/train_model/"
    ...
    trainer = MRCNERTrainer()
    model = model.to(device)
    trainer.train(model, optimizer, scheduler, train_dataloader, eval_dataloader, writer, device, model_dir)
